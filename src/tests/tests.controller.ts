...
export class TestsController {
  ...
  async getTestQuestions(@Param('testId') testId: number) {
    ...
    const template = await this.testTemplateService.getTemplateWithQuestions(testTemplateId);

    if (!template) {
      throw new NotFoundException(`Test Template with id ${testTemplateId} not found`);
    }
